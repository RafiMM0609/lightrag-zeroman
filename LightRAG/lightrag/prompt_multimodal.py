...
5. DESCRIPTION (`description`, ≤ 500 words, natural prose — not bullets)
   ...
   - If the image contains a matrix/approval table, convert it into a Markdown table inside the description. Ensure that:
     * If there are merged cells (such as Requester or Value), rewrite/repeat their values in each corresponding row.
     * Replace dot symbols (•) with the word 'Yes' or 'Diperlukan', and leave empty cells as 'No' or '-'.
     * Ensure sub-headers 'Approver I' and 'Approver II' are clearly connected to their respective column names.
