...
const LoginPage = () => {
  ...
  const { login, isAuthenticated, webuiLogoType, webuiTitle } = useAuthStore()
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      login(
        ...
        response.webui_logo_type || null,
        response.webui_logo_url || null,
        response.webui_theme_color || null
      )
        ...
        <CardHeader className="flex items-center justify-center space-y-2 pb-8 pt-6">
          <div className="flex flex-col items-center space-y-4 w-full">
            <div className="flex items-center justify-center">
              <BrandLogo className="h-10" iconSizeClassName="size-10" />
            </div>
            {webuiLogoType !== 'zeroman' && (
              <div className="text-center space-y-2">
                <h1 className="text-3xl font-bold tracking-tight">{webuiTitle || 'LightRAG'}</h1>
              </div>
            )}
            <div className="text-center space-y-2">
