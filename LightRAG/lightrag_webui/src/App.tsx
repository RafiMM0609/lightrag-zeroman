...
function App() {
  ...
  const webuiThemeColor = useAuthStore(state => state.webuiThemeColor)
  const webuiLogoType = useAuthStore(state => state.webuiLogoType)
  const webuiTitle = useAuthStore(state => state.webuiTitle)

  // Dynamic brand color application
  useEffect(() => {
    if (webuiThemeColor) {
      document.documentElement.style.setProperty('--brand-accent', webuiThemeColor)
      
      // Calculate dark hover/border color dynamically
      ...
      try {
        const darkColor = darkenColor(webuiThemeColor, 15)
        document.documentElement.style.setProperty('--brand-accent-dark', darkColor)
      } catch (e) {
        document.documentElement.style.setProperty('--brand-accent-dark', webuiThemeColor)
      }
    } else {
      // Revert to defaults (emerald)
      ...
      try {
        ...
        if (!status.auth_configured && status.access_token) {
          useAuthStore.getState().login(
            ...
            status.webui_logo_type || null,
            status.webui_logo_url || null,
            status.webui_theme_color || null
          );
        } else if (token && (status.core_version || status.api_version || status.webui_title || status.webui_description || status.webui_logo_type)) {
            ...
            <header className="border-border/40 bg-background/95 supports-[backdrop-filter]:bg-background/60 sticky top-0 z-50 flex h-10 w-full border-b px-4 backdrop-blur">
              <div className="min-w-[200px] w-auto flex items-center">
                <a href={webuiPrefix} className="flex items-center gap-2">
                  ...
                  {webuiLogoType !== 'zeroman' && (
                    <span className="font-bold md:inline-block">
                      {webuiTitle || SiteInfo.name}
                    </span>
                  )}
                </a>
              </div>
