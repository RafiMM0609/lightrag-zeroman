...
interface AuthState {
  ...
  webuiLogoType: string | null;
  webuiLogoUrl: string | null;
  webuiThemeColor: string | null;
  ...
  login: (token: string, isGuest?: boolean, coreVersion?: string | null, apiVersion?: string | null, webuiTitle?: string | null, webuiDescription?: string | null, webuiLogoType?: string | null, webuiLogoUrl?: string | null, webuiThemeColor?: string | null) => void;
  logout: () => void;
  setVersion: (coreVersion: string | null, apiVersion: string | null) => void;
  setBrandConfig: (webuiTitle: string | null, webuiDescription: string | null, webuiLogoType: string | null, webuiLogoUrl: string | null, webuiThemeColor: string | null) => void;
...
const useBackendStateStoreBase = create<BackendState>()((set, get) => ({
  ...
  check: async () => {
    ...
    if (health.status === 'healthy') {
      ...
      // Update custom title and brand information if health check returns it
      if ('webui_title' in health || 'webui_description' in health || 'webui_logo_type' in health) {
        useAuthStore.getState().setBrandConfig(
          'webui_title' in health ? (health.webui_title ?? null) : null,
          'webui_description' in health ? (health.webui_description ?? null) : null,
          'webui_logo_type' in health ? (health.webui_logo_type ?? null) : null,
          'webui_logo_url' in health ? (health.webui_logo_url ?? null) : null,
          'webui_theme_color' in health ? (health.webui_theme_color ?? null) : null
        );
      }
...
const initAuthState = (): { isAuthenticated: boolean; isGuestMode: boolean; coreVersion: string | null; apiVersion: string | null; username: string | null; webuiTitle: string | null; webuiDescription: string | null; webuiLogoType: string | null; webuiLogoUrl: string | null; webuiThemeColor: string | null; lastTokenRenewal: string | null; tokenExpiresAt: number | null } => {
  ...
  const webuiLogoType = localStorage.getItem('LIGHTRAG-WEBUI-LOGO-TYPE') || 'default';
  const webuiLogoUrl = localStorage.getItem('LIGHTRAG-WEBUI-LOGO-URL');
  const webuiThemeColor = localStorage.getItem('LIGHTRAG-WEBUI-THEME-COLOR');
  ...
  if (!token) {
    return {
      ...
      webuiLogoType: webuiLogoType,
      webuiLogoUrl: webuiLogoUrl,
      webuiThemeColor: webuiThemeColor,
...
export const useAuthStore = create<AuthState>(set => {
  ...
  return {
    ...
    webuiLogoType: initialState.webuiLogoType,
    webuiLogoUrl: initialState.webuiLogoUrl,
    webuiThemeColor: initialState.webuiThemeColor,
    lastTokenRenewal: initialState.lastTokenRenewal,
    tokenExpiresAt: initialState.tokenExpiresAt,

    login: (token, isGuest = false, coreVersion = null, apiVersion = null, webuiTitle = null, webuiDescription = null, webuiLogoType = null, webuiLogoUrl = null, webuiThemeColor = null) => {
      ...
      if (webuiLogoType) {
        localStorage.setItem('LIGHTRAG-WEBUI-LOGO-TYPE', webuiLogoType);
      } else {
        localStorage.removeItem('LIGHTRAG-WEBUI-LOGO-TYPE');
      }

      if (webuiLogoUrl) {
        localStorage.setItem('LIGHTRAG-WEBUI-LOGO-URL', webuiLogoUrl);
      } else {
        localStorage.removeItem('LIGHTRAG-WEBUI-LOGO-URL');
      }

      if (webuiThemeColor) {
        localStorage.setItem('LIGHTRAG-WEBUI-THEME-COLOR', webuiThemeColor);
      } else {
        localStorage.removeItem('LIGHTRAG-WEBUI-THEME-COLOR');
      }
    ...
    logout: () => {
      ...
      const webuiTitle = localStorage.getItem('LIGHTRAG-WEBUI-TITLE');
      const webuiDescription = localStorage.getItem('LIGHTRAG-WEBUI-DESCRIPTION');
      const webuiLogoType = localStorage.getItem('LIGHTRAG-WEBUI-LOGO-TYPE');
      const webuiLogoUrl = localStorage.getItem('LIGHTRAG-WEBUI-LOGO-URL');
      const webuiThemeColor = localStorage.getItem('LIGHTRAG-WEBUI-THEME-COLOR');
    ...
    setBrandConfig: (webuiTitle, webuiDescription, webuiLogoType, webuiLogoUrl, webuiThemeColor) => {
      // Update localStorage
      if (webuiTitle) {
        localStorage.setItem('LIGHTRAG-WEBUI-TITLE', webuiTitle);
      } else {
